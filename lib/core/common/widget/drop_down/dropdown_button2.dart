...
typedef OnPreviousOptionCheckFn = void Function(bool isOpen);

/// Signature for the callback for the match function used for searchable dropdowns.
typedef SearchMatchFn<T> =
    bool Function(DropdownMenuItem<T> item, String searchValue);
...
class _DropdownMenuState<T> extends State<_DropdownMenu<T>> {
  ...
  Widget get _materialScrollBar => Theme(
    data: Theme.of(
      context,
    ).copyWith(scrollbarTheme: dropdownStyle.scrollbarTheme),
    child: Scrollbar(
...
class DropdownButton2<T> extends StatefulWidget {
  ...
  final OnPreviousOptionCheckFn? onPreviousOptionCheck;
...
class DropdownButton2State<T> extends State<DropdownButton2<T>>
  ...
  Rect _getRect() {
    ...
    final NavigatorState navigator = Navigator.of(
      context,
      rootNavigator: _dropdownStyle.useRootNavigator,
    );
  ...
  Widget build(BuildContext context) {
    ...
    if (items.isEmpty) {
      ...
      innerItemsWidget = Container(
        ...
        alignment: widget.alignment,
        child: widget.selectedValue ?? widget.hint,
        /*Text(
          widget.itemList[_selectedIndex],
          style: AppThemeState().textStyleMedium(ColorConstants.textColor, fontSize: FontConstants.font_14),
        ),*/
        /*
        // items[_selectedIndex ?? items.length -1],
        */
    ...
    Widget result = DefaultTextStyle(
      ...
      child:
          widget.customButton ??
          Container(
            padding: EdgeInsets.symmetric(horizontal: AppSizeConstants.size_10),
            ...
            child: Row(
              ...
              children: <Widget>[
                ...
                /*CommonWhiteCircleWithCenterIcon(
                  child: SvgPicture.asset(ImageConstants.icArrowDown),
                  onTap: () {},
                )*/
                /* IconTheme(
                  data: IconThemeData(
    ...
    return Semantics(
      ...
      child: Actions(
        ...
        child: InkWell(
          ...
          onTap: () {
            if (widget.onPreviousOptionCheck != null) {
              widget.onPreviousOptionCheck!(true);
            }
            if (_enabled && !widget.openWithLongPress) {
              _handleTap();
            }
          },
...
class DropdownButtonFormField2<T> extends FormField<T> {
  ...
  DropdownButtonFormField2({
    ...
    OnPreviousOptionCheckFn? onPreviousOptionCheck,
    TextStyle? style,
    bool isDense = true,
    ...
    // required IconStyleData iconStyleData,
               ...
               builder: (BuildContext context) {
                 return DropdownButtonHideUnderline(
                   child: DropdownButton2<T>._formField(
                     ...
                     onPreviousOptionCheck: onPreviousOptionCheck,
                     style: style,
                     isDense: isDense,
  ...
  static InputDecoration _getInputDecoration(
    ...
    return decoration ??
        InputDecoration(
          focusColor: buttonStyleData?.overlayColor?.resolve(<WidgetState>{
            WidgetState.focused,
          }),
          hoverColor: buttonStyleData?.overlayColor?.resolve(<WidgetState>{
            WidgetState.hovered,
          }),
        );
  }
